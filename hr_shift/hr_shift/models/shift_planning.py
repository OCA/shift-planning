...
class ShiftPlanning(models.Model):
    ...
    days_data = fields.Json(default={}, compute="_compute_days_data")
...
class ShiftPlanningShift(models.Model):
    ...
    lines_data = fields.Json(default={}, compute="_compute_lines_data")
    state = fields.Selection(
        selection=[
    ...
    def _group_expand_template_id(self, templates, domain):
        return self.env["hr.shift.template"].search([], limit=None)
...
class ShiftPlanningLine(models.Model):
    ...
    def _constrain_template_id(self):
        for record in self.filtered("template_id"):
            if record.state == "holiday":
                ...
                raise UserError(
                    self.env._("This employee is on leave so can't be assigned to this shift")
                )
    ...
    def _is_on_leave(self):
        ...
        start_time = datetime.combine(
            pytz.utc.localize(self.start_time).astimezone(local_tz).date(),
            datetime.min.time(),
        )
        end_time = datetime.combine(
            pytz.utc.localize(self.end_time).astimezone(local_tz).date(),
            datetime.max.time(),
        )
        return bool(
